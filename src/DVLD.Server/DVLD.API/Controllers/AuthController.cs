...
using DVLD.Application.Features.Authentication.Refresh;
using DVLD.Application.Features.Roles.Get;
...
public class AuthController(ISender sender,IHttpContextAccessor accessor) : BaseController(sender,accessor)
{
    ...
    public async Task<IActionResult> GetCurrentUser()
    {
        ...
        var query = new GetRolePermissionsQuery(currentUser.Roles);
        var result = await Sender.Send(query);
        if(result.IsError)
            return HandleErrors(result.Errors);
        currentUser.Permissions = result.Value;
    ...
    public async Task<IActionResult> Refresh()
    {
        ...
        var cmd = new CreateRefreshTokenCommand(refreshToken, createdByIp, userAgent);
